...
type CreateConnectorRenderer = React.ComponentType<React.PropsWithChildren<ConnectorComponentProps>>;
...
const CreateConnectorWidget: React.FunctionComponent<React.PropsWithChildren<CreateConnectorWidgetProps>> = observer(
  props => {
    const {
      element,
      onKeepAlive,
...
export const withCreateConnector = <P extends WithCreateConnectorProps & ElementProps>(
  ...
  const Component: React.FunctionComponent<React.PropsWithChildren<
    Omit<P, keyof WithCreateConnectorProps>
  >> = props => {
