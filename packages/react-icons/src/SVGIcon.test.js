...
test('height and width are set from size', () => {
  const view = shallow(<SVGIcon config={iconDef} size={IconSize.sm} />);
...
test('title is rendered', () => {
  ...
  const view = shallow(<SVGIcon config={iconDef} title={title} />);
  expect(view.find('title').text()).toBe(title);
});

test('aria-labelledby matches title id', () => {
  const view = shallow(<SVGIcon config={iconDef} title="icon title" />);
...
test('ids should be unique for each rendered icon', () => {
  const first = shallow(<SVGIcon config={iconDef} title="icon title" />);
  const second = shallow(<SVGIcon config={iconDef} title="icon title" />);
...
test('additional props should be spread to the root svg element', () => {
  const view = shallow(<SVGIcon config={iconDef} data-testid="icon" />);
