...
export const ComposableTableStickyColumnsAndHeader: React.FunctionComponent = () => {
  ...
  const facts: Fact[] = Array.from({ length: 25 }, (_, index) => ({
    name: `Fact ${index + 1}`,
    state: `State ${index + 1}`,
  ...
  const Header = (
    <Masthead>
      ...
      <MastheadContent>header-tools</MastheadContent>
    </Masthead>
  );
    ...
    <Toolbar>
      <ToolbarContent>
        <ToolbarItem variant="search-filter">
          <SearchInput aria-label="search input example" onChange={() => {}} value="test toolbar" onClear={() => {}} />
        </ToolbarItem>
      </ToolbarContent>
    </Toolbar>
  );
