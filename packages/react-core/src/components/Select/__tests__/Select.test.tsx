...
describe('select with placeholder', () => {
  test('applies the placeholder class when not selected', () => {
    ...
    expect(screen.getByRole('button', { name: 'Options menu' }).className).toContain('pf-m-placeholder');
  });
  test('does not apply the placeholder class when selected', () => {
    ...
    expect(screen.getByRole('button', { name: 'Options menu' }).className).not.toContain('pf-m-placeholder');
  });
});
