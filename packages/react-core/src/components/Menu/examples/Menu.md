...
class MenuWithDrilldown extends React.Component {
  constructor(props) {
    ...
    this.setHeight = (menuId, height) => {
      if (this.state.menuHeights[menuId] === undefined) {
        this.setState({
          menuHeights: {
...
class MenuWithDrilldown extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      menuDrilledIn: ['rootMenu', 'drilldownMenuStart'],
      drilldownPath: ['group:start_rollout', 'group:app_grouping'],
      menuHeights: { rootMenu: 216 },
      activeMenu: 'drilldownMenuStartGrouping'
    };
    this.drillIn = (fromMenuId, toMenuId, pathId) => {
      this.setState({
        menuDrilledIn: [...this.state.menuDrilledIn, fromMenuId],
    ...
    this.setHeight = (menuId, height) => {
      if (
        this.state.menuHeights[menuId] === undefined ||
        (menuId !== 'rootMenu' && this.state.menuHeights[menuId] !== height)
      ) {
        this.setState({
          menuHeights: {
            ...this.state.menuHeights,
