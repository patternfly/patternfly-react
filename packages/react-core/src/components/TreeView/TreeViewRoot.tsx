...
export class TreeViewRoot extends React.Component<TreeViewRootProps> {
  ...
  componentDidMount() {
    if (canUseDOM) {
      window.addEventListener(
        'keydown',
        this.props.hasCheckboxs || this.props.hasSelectableNodes ? this.handleKeysCheckbox : this.handleKeys
      );
    }
    if (this.props.hasCheckboxs || this.props.hasSelectableNodes) {
      const firstToggle = this.treeRef.current.getElementsByClassName('pf-c-tree-view__node-toggle')[0] as HTMLElement;
      if (firstToggle) {
        firstToggle.tabIndex = 0;
      }
      if (this.props.hasCheckboxs) {
  ...
  render() {
    ...
    const { children, hasCheckboxs, hasGuides, variant, className, hasSelectableNodes, ...props } = this.props;
    return (
      <div
        className={css(
          styles.treeView,
