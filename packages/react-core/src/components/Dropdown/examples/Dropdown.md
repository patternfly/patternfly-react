  ...
  render() {
    ...
    return (
      <Dropdown
        ...
        toggle={
          <DropdownToggle id="toggle-id" onToggle={this.onToggle} toggleIndicator={CaretDownIcon} aria-haspopup>
            Dropdown
          </DropdownToggle>
        }
        isOpen={isOpen}
...
### Basic w/ aria controls & haspopup (authoring practices)
```js
import React from 'react';
import {
  Dropdown,
  ...
  render() {
    ...
    return (
      <Dropdown
        onSelect={this.onSelect}
        toggle={
          <DropdownToggle aria-controls="dropdown-menu" aria-haspopup id="toggle-id" onToggle={this.onToggle} toggleIndicator={CaretDownIcon}>
            Dropdown
          </DropdownToggle>
        }
        isOpen={isOpen}
...
class SimpleDropdown extends React.Component {
  constructor(props) {
    ...
    this.onToggle = isOpen => {
      ...
      setTimeout(function(){ console.log("Delayed for Jaws"); }, 3000);
    };
    this.onSelect = (event) => {
      this.setState({
        isOpen: !this.state.isOpen
