...
import { RowWrapper, RowWrapperRow, IExtraRowData } from '@patternfly/react-table';
import {
...
const tableConfirmationMapper = {
  TABLE_TOP: {
    hasConfirmationButtons: ({ isTableEditing, isFirstVisible }: EditableRowWrapperRow) => isTableEditing && isFirstVisible,
  ...
  TABLE_BOTTOM: {
    hasConfirmationButtons: ({ isTableEditing, isLastVisible }: EditableRowWrapperRow) => isTableEditing && isLastVisible,
    isTableConfirmation: () => true,
  ...
  ROW: {
    hasConfirmationButtons: ({ isEditing, isParentEditing, isLastVisibleParent, isChildEditing, isLastVisible }: EditableRowWrapperRow) =>
      isEditing &&
    ...
    areButtonsOnTop: ({ isLastVisible, isLastVisibleParent }: EditableRowWrapperRow) => isLastVisible || isLastVisibleParent,
    hasBoldBorder: () => false,
    getEditStyles: ({ isEditing }: EditableRowWrapperRow) => css(styles.tableEditableRow, isEditing && styles.modifiers.editing)
  },
  NO_CONFIRM_ROW: {
...
class RowWrapper extends React.Component<EditableRowWrapperProps, EditableRowWrapperState> {
  state = {
    hasConfirmationButtons: false,
    window: undefined as WindowDimensions,
    rowDimensions: undefined as ClientBoundingRect,
    ...RowWrapper.getDerivedStateFromProps(this.props)
  };
  element = React.createRef<HTMLTableRowElement>();
  tableElem: HTMLElement;
  static defaultProps = {
    onScroll: (_event: React.UIEvent<Element>) => {},
    onResize: (_event: React.UIEvent<Element>) => {},
  };
  ...
  setStateWith2dEquals = (newState: EditableRowWrapperState) => {
    this.setState(oldState =>
      Object.keys(newState).find(key => !shallowLeftSideEquals(newState[key as keyof EditableRowWrapperState], oldState[key as keyof EditableRowWrapperState])) ? newState : null
    );
  };
