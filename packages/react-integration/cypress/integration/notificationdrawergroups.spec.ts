describe('Notification Drawer Groups Demo Test', () => {
  ...
  it('Verify truncateTitle group title and tooltip', () => {
    cy.get(
      '#long-group-title > h1 > .pf-c-notification-drawer__group-toggle > :nth-child(1) > .pf-c-notification-drawer__group-toggle-title'
  ...
  it('Verify truncated with no truncateTitle prop group title and tooltip on long title', () => {
    cy.get(
      '#long-title-no-truncate-prop > h1 > .pf-c-notification-drawer__group-toggle  > :nth-child(1) > .pf-c-notification-drawer__group-toggle-title'
    ).then((tooltipLink: JQuery<HTMLDivElement>) => {
      cy.get('.pf-c-tooltip').should('not.exist');
      cy.wrap(tooltipLink)
        .trigger('mouseenter')
