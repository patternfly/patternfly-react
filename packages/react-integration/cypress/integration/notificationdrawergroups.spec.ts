describe('Notification Drawer Groups Demo Test', () => {
  ...
  it('Verify text in header status', () => {
    cy.get('.pf-v6-c-notification-drawer__header-status').contains('4 unread');
  });
  ...
  it('Verify keyboard events happen correctly', () => {
    ...
    cy.get('.pf-v6-c-notification-drawer__group').first().should('have.class', 'pf-m-expanded');
    cy.get('.pf-v6-c-notification-drawer__group-toggle').first().trigger('keydown', { key: 'Enter' });
  ...
  it('Verify truncateTitle for group title and no tooltip on short text', () => {
    cy.get(
      '#short-group-title > h1 > .pf-v6-c-notification-drawer__group-toggle > .pf-v6-c-notification-drawer__group-toggle-title'
  ...
  it('Verify truncateTitle group title and tooltip', () => {
    cy.get(
      '#long-group-title > h1 > .pf-v6-c-notification-drawer__group-toggle .pf-v6-c-notification-drawer__group-toggle-title'
  ...
  it('Verify truncated with no truncateTitle prop group title and tooltip on long title', () => {
    cy.get(
      '#long-title-no-truncate-prop > h1 > .pf-v6-c-notification-drawer__group-toggle .pf-v6-c-notification-drawer__group-toggle-title'
    ).then((tooltipLink: JQuery<HTMLDivElement>) => {
      cy.get('.pf-v6-c-tooltip').should('not.exist');
      cy.wrap(tooltipLink).trigger('mouseenter').get('.pf-v6-c-tooltip').should('exist');
      cy.wrap(tooltipLink).trigger('mouseleave');
