describe('Table Simple Test', () => {
  ...
  it('Verify tooltip info', () => {
    ...
    cy.get('.pf-v6-c-table__column-help-action').should('have.length', 2);
    // tooltip shouldn't exist yet
    cy.get('.pf-v6-c-tooltip').should('not.exist');
    // trigger tooltip
    cy.get('.pf-v6-c-table__column-help-action').first().click();
  ...
  it('Verify popover info', () => {
    ...
    // trigger popover
    cy.get('.pf-v6-c-table__column-help-action').eq(1).click();
