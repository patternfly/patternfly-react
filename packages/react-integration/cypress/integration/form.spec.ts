describe('Form Demo Test', () => {
  ...
  it('Verify labelInfo structure', () => {
    cy.get('#form-group-age .pf-v5-c-form__group-label.pf-m-info').should('exist');
    cy.get('#form-group-age .pf-v5-c-form__group-label-main').should('exist');
    cy.get('#form-group-age .pf-v5-c-form__group-label-info').should('exist');
  });

  it('Verify form allows correct input', () => {
    cy.get('#age').first().clear().type('5');
    cy.get('.pf-v5-c-helper-text').contains('Please write your age');
  });

  it('Verify form identifies input error', () => {
    cy.get('#age').first().clear().type('Something');
    cy.get('.pf-v5-c-helper-text').contains('Age has to be a number');
  });

  it('Verify section titles rendering with correct component', () => {
    cy.get('h4.pf-v5-c-form__section-title').contains('Title');
  });
  ...
  it('Verify form validates form group', () => {
    ...
    cy.get('.pf-v5-c-helper-text__item.pf-m-error').contains('Age must be a number');
  ...
  it('Verify form group label has no top spacer', () => {
    cy.get('.pf-v5-c-form__group-label').should('have.class', 'pf-m-no-padding-top');
  });
  ...
  it('Verify keypress can control the multi-select-typeahead', () => {
    ...
    cy.get('.pf-v5-c-label button').click();

    cy.get('[id*="pf-select-toggle-id"][id*="select-multi-typeahead-typeahead"]').type('New J');

    cy.get('.pf-v5-c-select__menu-item').should('exist').and('have.text', 'New Jersey');
