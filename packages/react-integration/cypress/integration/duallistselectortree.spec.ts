describe('Dual List Selector TreeDemo Test', () => {
  ...
  it('Verify expanding options', () => {
    ...
    cy.get('.pf-v5-c-dual-list-selector__list-item .pf-v5-c-dual-list-selector__item-toggle').eq(0).click();
    cy.get('.pf-v5-c-dual-list-selector__list-item').eq(0).should('have.class', 'pf-m-expanded');
    cy.get('.pf-v5-c-dual-list-selector__list').eq(0).find('li').should('have.length', 4);
  });

  it('Verify available search works', () => {
    cy.get('.pf-v5-c-dual-list-selector__list').eq(0).find('li').should('have.length', 4);
    cy.get('.pf-v5-c-dual-list-selector__tools-filter input').eq(0).type('Option 2');
    ...
    cy.get('.pf-v5-c-dual-list-selector__list-item .pf-v5-c-dual-list-selector__item-toggle').last().click();
    cy.get('.pf-v5-c-dual-list-selector__list').eq(0).find('li').should('have.length', 5);
  });
