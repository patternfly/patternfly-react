describe('Notification Drawer Lightweight Demo Test', () => {
  ...
  it('Verify text in header status', () => {
    cy.get('.pf-v6-c-notification-drawer__header-status').contains('2 unread alerts');
  });

  it('Verify 3 groups exist', () => {
    cy.get('.pf-v6-c-notification-drawer__group').then((groups) => expect(groups.length).to.equal(3));
  });

  it('Verify first and last list items are hidden', () => {
    cy.get('.pf-v6-c-notification-drawer__list').first().should('have.attr', 'hidden');
    cy.get('.pf-v6-c-notification-drawer__list').eq(1).should('not.have.attr', 'hidden', false);
    cy.get('.pf-v6-c-notification-drawer__list').last().should('have.attr', 'hidden');
  });

  it('Verify first item is expanded after click', () => {
    cy.get('.pf-v6-c-notification-drawer__group').first().click();
    cy.get('.pf-v6-c-notification-drawer__list').should('not.have.attr', 'hidden');
  });

  it('Verify list items in lightweight drawer are all in unread state', () => {
    cy.get('.pf-v6-c-notification-drawer__list-item').first().should('not.have.class', 'pf-m-read');
    cy.get('.pf-v6-c-notification-drawer__list-item').eq(1).should('not.have.class', 'pf-m-read');
    cy.get('.pf-v6-c-notification-drawer__list-item').eq(2).should('not.have.class', 'pf-m-read');
    cy.get('.pf-v6-c-notification-drawer__list-item').last().should('not.have.class', 'pf-m-read');
  });
  ...
  it('Verify list items severities', () => {
    ...
    cy.get('.pf-v6-c-notification-drawer__list-item').last().should('have.class', 'pf-m-success');
  });
  ...
  it('Verify timestamp in list items', () => {
    ...
    cy.get('.pf-v6-c-notification-drawer__list-item-timestamp').last().contains('30 minutes ago');
  });

  it('Verify all badges are marked unread', () => {
    cy.get('.pf-v6-c-notification-drawer__group-toggle-count').first().should('not.have.attr', 'pf-m-unread');
    cy.get('.pf-v6-c-notification-drawer__group-toggle-count').eq(1).should('not.have.attr', 'pf-m-unread');
    cy.get('.pf-v6-c-notification-drawer__group-toggle-count').last().should('not.have.attr', 'pf-m-unread');
  });
});
