describe('Notification Drawer Lightweight Demo Test', () => {
  ...
  it('Verify list items in lightweight drawer are all in unread state', () => {
    cy.get('.pf-c-notification-drawer__list-item').first().should('not.have.class', 'pf-m-read');
    cy.get('.pf-c-notification-drawer__list-item').eq(1).should('not.have.class', 'pf-m-read');
    cy.get('.pf-c-notification-drawer__list-item').eq(2).should('not.have.class', 'pf-m-read');
    cy.get('.pf-c-notification-drawer__list-item').last().should('not.have.class', 'pf-m-read');
  });
  ...
  it('Verify all badges are marked unread', () => {
    cy.get('.pf-c-notification-drawer__group-toggle-count').first().should('not.have.attr', 'pf-m-unread');
    cy.get('.pf-c-notification-drawer__group-toggle-count').eq(1).should('not.have.attr', 'pf-m-unread');
    cy.get('.pf-c-notification-drawer__group-toggle-count').last().should('not.have.attr', 'pf-m-unread');
  });
});
