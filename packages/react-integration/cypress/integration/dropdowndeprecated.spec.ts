describe('Dropdown Deprecated Demo Test', () => {
  it('Navigate to demo section', () => {
    cy.visit('http://localhost:3000/dropdown-deprecated-demo-nav-link');
  });
  ...
  it('Verify dropdown menu has alignment breakpoints', () => {
    cy.get('#dropdown > button').click();
    cy.get('#dropdown').should('have.class', 'pf-m-expanded');
    const menu = cy.get('#dropdown .pf-v6-c-dropdown__menu');
  ...
  it('Space opens panel, places focus on first element in panel that can receive focus', () => {
    ...
    cy.get('.pf-v6-c-dropdown__menu-item').last().trigger('keydown', { key: ' ' });
    cy.get('#dropdown').should('not.have.class', 'pf-m-expanded');
  ...
  it('Enter closes the panel', () => {
    cy.get('#dropdown > button').click();
    cy.get('.pf-v6-c-dropdown__menu-item').last().trigger('keydown', { key: 'Enter' });
    cy.get('#dropdown').should('not.have.class', 'pf-m-expanded');
  });

  it('When focus is in panel, space places focus on toggle', () => {
    cy.get('#dropdown > button').click();
    cy.get('.pf-v6-c-dropdown__menu').trigger('keydown', { key: ' ' });
    cy.focused().should('have.class', 'pf-v6-c-dropdown__toggle');
  });

  // Panel > Menu > Menu Item - Managing focus
  it('Tab closes panel, focus stays on toggle', () => {
    cy.get('.pf-v6-c-dropdown__menu-item').last().trigger('keydown', { key: 'Tab' });
    cy.get('.pf-v6-c-dropdown__toggle').last().should('not.have.class', 'pf-m-expanded');
    cy.focused().contains('Dropdown');
  });

  it('Shift + Tab closes panel, focus stays on toggle', () => {
    cy.get('#dropdown > button').click();
    cy.get('.pf-v6-c-dropdown__menu-item').last().trigger('keydown', { shiftKey: true, key: 'Tab' });
    cy.get('.pf-v6-c-dropdown__toggle').last().should('not.have.class', 'pf-m-expanded');
    cy.focused().contains('Dropdown');
  });
});
...
describe('Action Dropdown Demo Test', () => {
  ...
  it('Verify toggle dropdown', () => {
    cy.get('#action-toggle-id').should('have.class', 'pf-v6-c-dropdown__toggle-button');
  });
  ...
  it('When focus is in panel, space places focus on toggle', () => {
    cy.get('#action-dropdown button').last().click();
    cy.get('.pf-v6-c-dropdown__menu').trigger('keydown', { key: ' ' });
    cy.focused().should('have.class', 'pf-v6-c-dropdown__toggle-button');
  });
...
describe('Cog Dropdown Demo Test', () => {
  ...
  it('Verify toggle dropdown', () => {
    cy.get('#cog-toggle-id').should('have.class', 'pf-v6-c-dropdown__toggle-button');
  });
...
describe('Dropdown with menu on document body demo test', () => {
  ...
  it('Verify toggle dropdown', () => {
    cy.get('#dropdown-document-body > button').should('have.class', 'pf-v6-c-dropdown__toggle');
  });
  ...
  it('Verify dropdown menu is on document.body', () => {
    cy.get('body .pf-v6-c-dropdown').should('exist');
  });
