describe('Notification Badge Demo Test', () => {
  ...
  it('Verify notification badge is unread', () => {
    cy.get('.pf-c-notification-badge').first().should('have.class', 'pf-m-unread');
    cy.get('.pf-c-notification-badge').last().should('have.class', 'pf-m-attention');
  ...
  it('Verify notification badge is read after click', () => {
    ...
    cy.get('.pf-c-notification-badge').first().should('have.class', 'pf-m-read');
    cy.get('#notification-demo-badge2').click();
    cy.get('.pf-c-notification-badge').last().should('have.class', 'pf-m-read');
  });
});
