describe('Select in modal Test', () => {
  ...
  it('Verify Escape Press closes menus before modal', () => {
    cy.get('#modal-for-select-in-modal').then((modalButton: JQuery<HTMLButtonElement>) => {
      ...
      cy.get('.pf-v6-c-page').then(() => {
        cy.get('.pf-v6-c-modal-box')
          .then(() => {
            cy.get('#select-in-modal-select').then((selectToggle) => {
              cy.wrap(selectToggle).click();
              cy.get('.pf-v6-c-select .pf-v6-c-select__menu').should('exist');
              cy.get('body').type('{esc}');
              cy.get('.pf-v6-c-modal-box').should('exist');
              cy.get('.pf-v6-c-select .pf-v6-c-select__menu').should('not.exist');
            });
            cy.get('#select-in-modal-time-picker-input').then((timePicker) => {
              cy.wrap(timePicker).click();
              cy.get('.pf-v6-c-menu.pf-m-scrollable').should('exist');
              cy.get('body').type('{esc}');
              cy.get('.pf-v6-c-modal-box').should('exist');
              cy.get('.pf-v6-c-menu.pf-m-scrollable').should('not.exist');
            });
          })
          .then(() => {
            cy.get('.pf-v6-c-modal-box').should('exist');
