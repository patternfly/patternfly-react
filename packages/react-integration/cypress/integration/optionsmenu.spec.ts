describe('Login Page Demo Test', () => {
  ...
  it('Verify initial state', () => {
    cy.get('#options-menu-single-option-example')
      .find('.pf-v6-c-options-menu__toggle-text')
      .then((toggleText) => expect(toggleText.text()).to.equal('Options menu'));
    cy.get('#options-menu-single-option-example').find('.pf-v6-c-options-menu__toggle-icon').should('exist');
    ...
    cy.get('#options-menu-modified-example').find('.pf-v6-c-options-menu__toggle.pf-m-text.pf-m-plain').should('exist');
  ...
  it('Verify event handlers fire correctly', () => {
    cy.get('#options-menu-single-option-example')
      .find('.pf-v6-c-options-menu__toggle')
      .then((toggleButton: JQuery<HTMLButtonElement>) => {
        ...
        cy.get('#singleOption2').then((secondMenuItem: JQuery<HTMLButtonElement>) => {
          ...
          cy.get('#singleOption2')
            .find('.pf-v6-c-options-menu__menu-item-icon')
            .then((secondMenuItemIcon) => expect(secondMenuItemIcon).to.be.visible);
          cy.get('#singleOption1 .pf-v6-c-options-menu__menu-item-icon').should('not.exist');
        });
        cy.wrap(toggleButton).click();
        cy.get('#options-menu-single-option-example.pf-m-expanded').should('not.exist');
      });
    ...
    cy.get('#options-menu-modified-example')
      .find('.pf-v6-c-options-menu__toggle-button')
      .then((toggleButton: JQuery<HTMLButtonElement>) => {
        cy.wrap(toggleButton).click();
        ...
        cy.get('.pf-v6-c-options-menu__menu-item').first().should('be.focused');
      });
  });

  it('Verify disabled options menu', () => {
    cy.get('#options-menu-modified-disabled').find('.pf-v6-c-options-menu__toggle').should('be.disabled');
  });
});
