describe('Search Input Demo Test', () => {
  ...
  it('Disabled search input is disabled', () => {
    ...
    cy.get('#disabled-search .pf-c-input-group button').eq(0).should('be.disabled');
    cy.get('#disabled-search .pf-c-input-group button').eq(1).should('be.disabled');
  });
  ...
  it('Verify search input with hint', () => {
    ...
    cy.get('#hinted-search .pf-c-text-input-group__text-input').eq(1).should('not.be.disabled');
  });
  ...
  it('Verify search input and its handlers work', () => {
    ...
    cy.get('#enabled-search .pf-c-text-input-group__group button').last().click();
    cy.get('#enabled-search .pf-c-badge').should('have.text', '2 / 3');
    cy.get('#enabled-search .pf-c-text-input-group__group button').first().click();
    cy.get('#enabled-search .pf-c-badge').should('have.text', '1 / 3');
  ...
  it('Verify advanced search and its handlers work', () => {
    ...
    cy.get('#enabled-search .pf-c-input-group > button').eq(0).click();
    cy.get('#enabled-search .pf-c-panel').should('be.visible');

    cy.get('#enabled-search .pf-c-form-control').eq(0).type('test');
    ...
    cy.get('#enabled-search .pf-c-form-control').eq(1).should('have.value', 'hi');
    cy.get('#enabled-search .pf-c-form-control').eq(2).should('have.value', 'another test');

    cy.get('#enabled-search .pf-c-form__actions button').eq(1).click();
    cy.get('#enabled-search .pf-c-text-input-group__text-input').should('have.value', '');
    cy.get('#enabled-search .pf-c-form-control').eq(1).should('have.value', '');
    cy.get('#enabled-search .pf-c-form-control').eq(2).should('have.value', '');

    cy.get('#enabled-search .pf-c-form-control').eq(0).type('test');
    cy.get('#enabled-search .pf-c-form__actions button').eq(0).click();
