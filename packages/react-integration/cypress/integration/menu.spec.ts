describe('Menu Test', () => {
  ...
  it('Verify Icons Menu', () => {
    cy.get('#menu-with-icons.pf-v6-c-menu').should('exist');

    cy.get('.pf-v6-c-menu__item-icon > svg').eq(0).should('have.attr', 'id', 'code-branch-icon');

    cy.get('.pf-v6-c-menu__item-icon > svg').eq(1).should('have.attr', 'id', 'layer-group-icon');

    cy.get('.pf-v6-c-menu__item-icon > svg').eq(2).should('have.attr', 'id', 'cube-icon');
  });

  it.skip('Verify Flyout Menu', () => {
    cy.get('.pf-v6-c-menu.pf-m-flyout').should('exist');

    cy.get('#edit').click();
  });

  it('Verify Filterable Menu', () => {
    cy.get('#filterable-menu.pf-v6-c-menu > div').first().should('have.class', 'pf-v6-c-menu__search');

    cy.get('.pf-v6-c-text-input-group__text-input').type('Action 1');
  ...
  it('Verify Menu with Separator', () => {
    cy.get('#menu-with-separators.pf-v6-c-menu > ul > li').eq(2).should('have.attr', 'id', 'separator');
  });

  it('Verify Menu with Titled Groups', () => {
    cy.get('#group-1.pf-v6-c-menu__group > h1').should('contain', 'Group 1');
    cy.get('#group-2.pf-v6-c-menu__group > h1').should('contain', 'Group 2');
    cy.get('#group-3.pf-v6-c-menu__group > div > h1').should('contain', 'Group 3');
    cy.get('#group-4.pf-v6-c-menu__group > div > h1.my-awesome-style').should('contain', 'Group 4');
  });
  ...
  it('Verify Single Select Menu', () => {
    cy.get('#single-select-menu.pf-v6-c-menu').should('exist');
  ...
  it('Verify Multi Select Menu', () => {
    cy.get('#multi-select-menu.pf-v6-c-menu').should('exist');
  ...
  it('Verify Footer Menu', () => {
    cy.get('#menu-footer.pf-v6-c-menu').should('exist');

    cy.get('#menu-footer .pf-v6-c-menu__footer').should('exist');
  });

  it('Verify View More Menu', () => {
    cy.get('#menu-view-more.pf-v6-c-menu').should('exist');

    cy.get('#menu-view-more .pf-v6-c-menu__list').find('.pf-v6-c-menu__list-item').should('have.length', 4);

    cy.get('#menu-view-more .pf-v6-c-menu__list-item').last().should('have.text', 'View more').click();

    cy.get('#menu-view-more .pf-v6-c-menu__list').find('.pf-v6-c-menu__list-item').should('have.length', 7);
  });
