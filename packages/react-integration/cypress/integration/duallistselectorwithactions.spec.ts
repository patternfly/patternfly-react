describe('Dual List Selector With Actions Demo Test', () => {
  ...
  it('Verify custom aria-labels, status, and titles', () => {
    cy.get('.pf-m-available .pf-v6-c-dual-list-selector__title-text').contains('Demo available options');
    cy.get('.pf-m-available .pf-v6-c-dual-list-selector__status-text').contains('4 available');
    cy.get('.pf-m-available .pf-v6-c-dual-list-selector__tools-filter input').should(
      'have.attr',
      'aria-label',
      'Demo available options search'
    );
    cy.get('.pf-m-available .pf-v6-c-dual-list-selector__list').should(
      'have.attr',
      'aria-labelledby',
      'dual-list-selector-demo-available-pane-status'
    );

    cy.get('.pf-v6-c-dual-list-selector__controls-item button').eq(1).should('have.attr', 'aria-label', 'Demo add all');
    ...
    cy.get('.pf-m-chosen .pf-v6-c-dual-list-selector__title-text').contains('Demo chosen options');
    cy.get('.pf-m-chosen .pf-v6-c-dual-list-selector__status-text').contains('0 chosen');
    cy.get('.pf-m-chosen .pf-v6-c-dual-list-selector__tools-filter input').should(
      'have.attr',
      'aria-label',
      'Demo chosen options search'
    );
    cy.get('.pf-m-chosen .pf-v6-c-dual-list-selector__list').should('not.have.attr', 'aria-labelledby');
  });
  ...
  it('Verify selecting and choosing options', () => {
    ...
    cy.get('.pf-m-available .pf-v6-c-dual-list-selector__status-text').contains('2 available');
    cy.get('.pf-m-chosen .pf-v6-c-dual-list-selector__status-text').contains('2 chosen');
  ...
  it('Verify choosing all options', () => {
    ...
    cy.get('.pf-m-available .pf-v6-c-dual-list-selector__status-text').contains('0 available');
    cy.get('.pf-m-chosen .pf-v6-c-dual-list-selector__status-text').contains('4 chosen');
  });

  it('Verify sort works', () => {
    cy.get('.pf-m-chosen .pf-v6-c-dual-list-selector__list-item').last().contains('Option 3');
