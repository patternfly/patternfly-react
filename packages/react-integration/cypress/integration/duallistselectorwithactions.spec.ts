describe('Dual List Selector With Actions Demo Test', () => {
  ...
  it('Verify default value content', () => {
    cy.get('.pf-c-dual-list-selector__list').first().should('have.value', '');
  ...
  it('Verify selecting options', () => {
    ...
    cy.get('.pf-c-dual-list-selector__list-item').eq(0).click();
    cy.get('.pf-c-dual-list-selector__list-item .pf-m-selected').should('exist');
    cy.get('.pf-c-dual-list-selector__list-item').eq(1).click();
  ...
  it('Verify selecting and choosing options', () => {
    ...
    cy.get('.pf-c-dual-list-selector__list').eq(0).find('li').should('have.length', 3);
    cy.get('.pf-c-dual-list-selector__list').eq(1).find('li').should('have.length', 1);
    cy.get('.pf-c-dual-list-selector__list-item').eq(1).click();
    cy.get('.pf-c-dual-list-selector__controls-item').eq(0).click();
    ...
    cy.get('.pf-c-dual-list-selector__list').eq(0).find('li').should('have.length', 2);
    cy.get('.pf-c-dual-list-selector__list').eq(1).find('li').should('have.length', 2);
  });

  it('Verify removing all options', () => {
    cy.get('.pf-c-dual-list-selector__controls-item').eq(2).click();
    cy.get('.pf-c-dual-list-selector__list').eq(0).find('li').should('have.length', 4);
    cy.get('.pf-c-dual-list-selector__list').eq(1).find('li').should('have.length', 0);
    cy.get('.pf-m-available .pf-c-dual-list-selector__status-text').contains('4 available');
    cy.get('.pf-m-chosen .pf-c-dual-list-selector__status-text').contains('0 chosen');
  });
  ...
  it('Verify choosing all options', () => {
    ...
    cy.get('.pf-c-dual-list-selector__list').eq(0).find('li').should('have.length', 0);
    cy.get('.pf-c-dual-list-selector__list').eq(1).find('li').should('have.length', 4);
  ...
  it('Verify sort works', () => {
    cy.get('.pf-m-chosen .pf-c-dual-list-selector__list-item').last().contains('Option 3');
    cy.get('.pf-m-chosen .pf-c-dual-list-selector__tools-actions button').first().click();
    cy.get('.pf-m-chosen .pf-c-dual-list-selector__list-item').last().contains('Option 4');
  });

  it('Verify chosen search works', () => {
    cy.get('.pf-c-dual-list-selector__list').eq(1).find('li').should('have.length', 4);
    cy.get('.pf-c-dual-list-selector__tools-filter input').eq(1).type('Option 1');
    cy.get('.pf-c-dual-list-selector__list').eq(1).find('li').should('have.length', 1);
  });
  ...
  it('Verify available search works', () => {
    cy.get('.pf-c-dual-list-selector__list').eq(0).find('li').should('have.length', 4);
    cy.get('.pf-c-dual-list-selector__tools-filter input').eq(0).type('Option 3');
    cy.get('.pf-c-dual-list-selector__list').eq(0).find('li').should('have.length', 1);
  });
