function checkFiltering() {
  cy.get('.pf-v6-c-form-control').type('run');
  cy.get('#Running').should('exist');
  cy.get('#Hewlett-Packard').should('not.exist');
  cy.get('.pf-v6-c-form-control').type('{backspace}{backspace}{backspace}degr');
...
describe('Select with Filtering Test', () => {
  ...
  it('Verify Checkbox Select with filtering chips', () => {
    ...
    cy.get('.pf-v6-c-select__toggle').contains('1').should('exist');
  });
  it('Verify Checkbox Select with filtering works', () => {
    checkFiltering();
    cy.get('.pf-v6-c-select__toggle').contains('2').should('exist');
  });
