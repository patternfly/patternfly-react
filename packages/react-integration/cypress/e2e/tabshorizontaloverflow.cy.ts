describe('Tab Demo Test', () => {
  ...
  it('Allows users to select a tab from the overflow menu', () => {
    ...
    ['Tab item 1', 'Tab item 2', 'Tab item 3', 'Tab item 4', 'Tab item 5', 'Tab item 6', 'Tab item 7', 'More'].forEach(
      (tab) => cy.contains('.pf-v5-c-tabs__link', tab).should('exist')
    );
    ...
    cy.contains('.pf-v5-c-tabs__link', 'More').click();
    ...
    cy.contains('.pf-v5-c-tabs__link', 'More').should('not.exist');
    cy.contains('.pf-v5-c-tabs__link', 'Tab item 9').should('exist');
    ...
    cy.contains('.pf-v5-c-tabs__link', 'More').should('exist');
  ...
  it('Adjusts tabs showing on resize', () => {
    ...
    ['Tab item 1', 'Tab item 2', 'Tab item 3', 'Tab item 4', 'Tab item 5', 'More'].forEach((tab) =>
      cy.contains('.pf-v5-c-tabs__link', tab).should('exist')
    );
    ...
    [
      ...
      'Tab item 10',
      'Tab item 11'
    ].forEach((tab) => cy.contains('.pf-v5-c-tabs__link', tab).should('exist'));
    cy.contains('.pf-v5-c-tabs__link', 'More').should('not.exist');
  });
  ...
  it('Allows overflow tab customization via HorizontalOverflowObject properties', () => {
    ...
    cy.viewport(800, 792);
    cy.contains('.pf-v5-c-tabs__link', 'More (5)').should('exist');
    cy.viewport(700, 792);
    cy.contains('.pf-v5-c-tabs__link', 'More').should('exist');
    cy.get('#toggle-show-count-overflow').click();
    ...
    cy.contains('.pf-v5-c-tabs__link', 'More').should('not.exist');
    cy.contains('.pf-v5-c-tabs__link', 'Overflow').should('exist');
    cy.contains('.pf-v5-c-tabs__link', 'Overflow').click();
    cy.contains('Tab item 9').click();
    cy.contains('.pf-v5-c-tabs__link', 'Overflow').should('not.exist');
    cy.contains('Tab item 1').click();
    cy.contains('.pf-v5-c-tabs__link', 'Overflow').should('exist');
    cy.get('#toggle-change-toggle-text').click();

    // verify that the overflow tab aria label can be updated
    cy.contains('.pf-v5-c-tabs__link', 'More').should('not.have.attr', 'aria-label');
    cy.get('#toggle-add-aria-label').click();
    cy.contains('.pf-v5-c-tabs__link', 'More').should('have.attr', 'aria-label', 'Overflow aria label');
    cy.get('#toggle-add-aria-label').click();
  });
});
