...
export class TableStickyHeaderDemo extends React.Component<TableProps, TableState> {
  ...
  render() {
    ...
    return (
      <React.Fragment>
        {(
          <Table caption="Sticky Header Table" isStickyHeader cells={['Title', 'Body']} rows={res.map((post: { title: any; body: any; }) => [post.title, post.body])} aria-label="Sticky Header Table Demo">
            <TableHeader />
            <TableBody />
          </Table>
        )}
