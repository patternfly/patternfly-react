...
import { getLegendTooltip } from '../Tooltip';
import { getLineSeries } from '../Line';
import { getSankeyTooltip, getSankeySeries, SankeyOption } from '../Sankey';
 ...
 * @beta
 */
export interface PatternflyEChartsProps {
  /**
   ...
   *
   * @propType string
   * @example <PatternflyECharts nodeSelector="html" />
   * @example <PatternflyECharts nodeSelector="#main" />
   * @example <PatternflyECharts nodeSelector=".chr-scope__default-layout" />
   */
  nodeSelector?: string;
  /**
   ...
   */
  option?: EChartsOption | SankeyOption;
  /**
...
export const PatternflyECharts: React.FunctionComponent<PatternflyEChartsProps> = ({
  className,
  height,
  id,
  ...
  const getTooltip = useCallback(
    (series: any[], tooltipType: string, isSkeleton: boolean) => {
      ...
      if (isSkeleton) {
        ...
        return getLegendTooltip(series, option);
      }
  ...
  React.useEffect(() => {
    ...
    echart.current?.setOption({
      ...option,
      ...(isLegendTooltip && { tooltip: getTooltip(series, tooltipType, isSkeleton) }),
      series
    });
...
PatternflyECharts.displayName = 'PatternflyECharts';
