...
class TaggingStoryComponent extends React.Component {
    ...
    this.state = {
      tags: [
        ...
        },
        {
          ...
          singleValue: true,
          values: [
            { description: 'Pepa', id: 11 },
            { description: 'Franta', id: 12 }
          ]
        },
        {
          description: 'Number',
  ...
  onTagValueChange = ({ tagValue, tagCategory }) => {
    ...
    this.setState({
      assignedTags: [...filteredTags, { ...tagCategory, tagValues: [tagValue] }]
    });
  ...
  onTagMultiValueChange = ({ tagValue, tagCategory }) => {
    const assignedTags = [...this.state.assignedTags];
    const filteredTags = assignedTags.filter(tag => tag.id !== tagCategory.id);
    const selectedItem = assignedTags.find(
      tag => tag.id === tagCategory.id
    ) || { tagValues: [] };
    const oldValues = selectedItem.tagValues.filter(
      value => value.id !== tagValue.id
    );
    this.setState({
      assignedTags: [
        ...filteredTags,
  ...
  onTagDeleteClick = ({ tagCategory, tagValue }) => {
    ...
    const filteredTagValues = selectedItem.tagValues.filter(
      value => value.id !== tagValue.id
    );
