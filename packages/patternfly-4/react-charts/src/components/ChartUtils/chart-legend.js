...
const getColumnWidths = (props, data) => {
  ...
  return columns.reduce((memo, curr, index) => {
    const lengths = dataByColumn[curr].map(d => d.textSize.width + d.size + d.symbolSpacer + gutterWidth);
...
const getRowHeights = (props, data) => {
  ...
  return keys(dataByRow).reduce((memo, curr, index) => {
    ...
    const lengths = rows.map(d => d.textSize.height + d.symbolSpacer + gutterHeight);
    memo[index] = Math.max(...lengths);
    return memo;
  }, []);
};
...
const getTitleProps = (props, borderProps) => {
  ...
  const getPadding = () => borderPadding[titleOrientation] + (padding || 0);
