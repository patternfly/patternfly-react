...
class OneHandlerLoginPage extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  handleChangeText = (value, event) => {
    const match = event.currentTarget.name.match(/^pf\-login\-(.*)\-id$/)
    if (!match) {
      return;
    }
  ...
  render() {
    ...
    const loginForm = (
      <LoginForm
        ...
        onChangeUsername={this.handleChangeText}
...
export default OneHandlerLoginPage;
