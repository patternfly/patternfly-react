...
# yarn run lerna publish prerelease --dist-tag=prerelease-v5 --yes 2>&1 | tee lerna-output.txt

# use lerna command below for dry run
yarn run lerna version prerelease --yes --no-git-tag-version --no-push | tee lerna-output.txt
