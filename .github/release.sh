...
echo "https://${GIT_USERNAME}:${GH_TOKEN_ADMIN}@${REPO}" > ~/.git-credentials
...
yarn run lerna publish --conventional-commits --create-release=github --no-verify-access --yes | tee lerna-output.txt
